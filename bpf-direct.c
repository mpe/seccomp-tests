...
static void emulator(int nr, siginfo_t *info, void *void_context)
{
	...
	if (!ctx)
		return;
	syscall = ctx->uc_mcontext.gp_regs[REG_SYSCALL];
	buf = (char *) ctx->uc_mcontext.gp_regs[REG_ARG1];
	len = (size_t) ctx->uc_mcontext.gp_regs[REG_ARG2];

	if (syscall != __NR_write)
		return;
	if (ctx->uc_mcontext.gp_regs[REG_ARG0] != STDERR_FILENO)
		return;
	/* Redirect stderr messages to stdout. Doesn't handle EINTR, etc */
	ctx->uc_mcontext.gp_regs[REG_RESULT] = -1;
	if (write(STDOUT_FILENO, "[ERR] ", 6) > 0) {
		bytes = write(STDOUT_FILENO, buf, len);
		ctx->uc_mcontext.gp_regs[REG_RESULT] = bytes;
	}
	return;
}
